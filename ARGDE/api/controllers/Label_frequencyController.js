module.exports = {
	createPreLabels: function(params){
		...
		+Argde.attributes.aidr_class_label.columnName+" is not NULL;",function(err, classes){
			...
			{
				var class_names = [];
				classes.rows.forEach(function(a_class){
					class_names.push(String(a_class['aidr_class_label']));
				});

				Argde.query("select distinct "+Argde.attributes.sentiment.columnName+" from "+Argde.tableName+" where "
				+Argde.attributes.sentiment.columnName+" is not NULL;",function(err, sentiments){
					if(err)
					{
						sails.log.error("Error name: "+err.name+"	 "+"Error code: "+err.code);
					}
					else
					{
						...
						function(err, damage_classes){
							...
							{
								var damage_values = [];
								damage_classes.rows.forEach(function(value){
									damage_values.push(String(value[String(Argde.attributes.image_damage_class.columnName)]));
								});
								...
								var query_p1 = "select count("+Argde.attributes.updatedAt.columnName+") from "+Argde.tableName
								+" where "+Argde.attributes.updatedAt.columnName+" >= timestamp '"+_date+"'+interval '";
						    ...
						    {
						      sql_query = query_p1+(i-1)+" minutes'"+query_p2+i+" minutes' and "
						            ...
						            if(err)
						            {
						              sails.log.error("Error name: "+_err.name+"	"+"Error code: "+_err.code);
												...
												else
												{
													...
													Argde.query("select timestamp '"+_date+"'+interval '"+(i-1)+" minutes';",
													function(errVal, newHour){
														if(errVal)
														{
